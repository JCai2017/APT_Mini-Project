...
from upload import MarkerImageHandler
from upload import GeoView
from upload import Geo_Data
	...
	def get(self):
		...
		if user:
			...
			for elem in createdStreams:
				viewCount = View.query(View.stream == elem.key).count(
																	limit=None)
				imgCount = Image.query(Image.stream == elem.key).count(
																	limit=None)
			...
			for elem in lst:
				...
				viewCount = View.query(View.stream == elem.stream).count(
																	limit=None)
				imgCount = Image.query(Image.stream == elem.stream).count(
																	limit=None)
		...
		for entry in checkedC:
			...
			for st in userData.created:
				if st.name == entry:
					...
					View_list = View.query(View.stream == delstream.key)
					for viewsTobeDelete in View_list:
						viewsTobeDelete.key.delete()

					Tag_list = Tag.query(Tag.stream == delstream.key)
					for tagsTobeDelete in Tag_list:
						tagsTobeDelete.key.delete()
					break
				i = i + 1
...
class Subscribe(webapp2.RequestHandler):
	def post(self):
		...
		subscribersList = Subscriber.query(Subscriber.stream == streamKey)\
			.fetch()
